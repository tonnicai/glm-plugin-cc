...
export const BROKER_ENDPOINT_ENV = "OLLAMA_COMPANION_APP_SERVER_ENDPOINT";
export const BROKER_BUSY_RPC_CODE = -32001;

// Run the Ollama harness under an isolated CODEX_HOME so it never touches the
// user's own Codex/Claude configuration (~/.codex or ~/.claude).
const OLLAMA_CODEX_HOME = process.env.OLLAMA_CODEX_HOME || `${process.env.HOME || process.env.USERPROFILE || "."}/.codex-ollama`;
...
class SpawnedCodexAppServerClient extends AppServerClientBase {
  ...
  async initialize() {
    this.proc = spawn("codex", ["app-server"], {
      cwd: this.cwd,
      env: { ...(this.options.env ?? process.env), CODEX_HOME: OLLAMA_CODEX_HOME },
