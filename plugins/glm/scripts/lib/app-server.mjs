...
export const BROKER_ENDPOINT_ENV = "GLM_COMPANION_APP_SERVER_ENDPOINT";
export const BROKER_BUSY_RPC_CODE = -32001;

// Run the GLM harness under an isolated CODEX_HOME so it never touches the
// user's own Codex/Claude configuration (~/.codex or ~/.claude).
const GLM_CODEX_HOME = process.env.GLM_CODEX_HOME || `${process.env.HOME || process.env.USERPROFILE || "."}/.codex-glm`;
...
class SpawnedCodexAppServerClient extends AppServerClientBase {
  ...
  async initialize() {
    this.proc = spawn("codex", ["app-server"], {
      cwd: this.cwd,
      env: { ...(this.options.env ?? process.env), CODEX_HOME: GLM_CODEX_HOME },
