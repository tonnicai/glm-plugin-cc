...
- Do not ask the subagent to inspect files, monitor progress, poll `/glm:status`, fetch `/glm:result`, call `/glm:cancel`, summarize output, or do follow-up work of its own.
...
- If the helper reports that Codex is missing or unauthenticated, stop and tell the user to run `/glm:setup`.
